...
1. 确认 `<小说目录>/items.json` 存在且 `<小说目录>/archive/items.json` 不存在
2. 将 `<小说目录>/items.json` 移动到 `<小说目录>/archive/items.json`
3. 读取 `archive/items.json`、`chapter_summaries.json`、`characters.json`
4. **非"特殊"类型**：按 TYPE_MAP 映射，未映射的删除
5. **评分"特殊"类型**：
...
6. **人工复核待定项**：输出 `items_pending.md`，用户决定保留/删除
7. **去人名**：兵器/防具/饰品/暗器/坐骑去掉人名前缀，同类型同名合并
8. **道具图鉴过滤**：再过一遍保留清单——这个道具是否有资格列入图鉴？有名号、有故事、有特殊属性的才留，普通货色删
9. 补全 `rarity_tier`："未知"默认「寻常凡品」
10. 写入 `<小说目录>/items.json` + `<小说目录>/items_summary.md`
