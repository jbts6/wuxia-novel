...
- 人物和武功都必须写 `power_rank`。八级从低到高固定为：平平无奇、初窥门径、略有小成、登堂入室、炉火纯青、出神入化、登峰造极、返璞归真。章节草稿写本章暂定值；plot/martial 的 keep 补丁写全书证据支持的巅峰值，并覆盖暂定值。
- 武功只保留 `power_rank`，物品不保留稀有度字段。
...
[
  {
    ...
    "decisions": [
      { "entry_ref": "plot:character:001", "action": "keep", "patch": { "level": "核心", "power_rank": "登堂入室" } },
  ...
  },
  {
    ...
    "decisions": [
      { "entry_ref": "martial:skill:001", "action": "keep", "patch": { "type": "内功", "power_rank": "炉火纯青" } },
...
`recall:<category>` 和 `supplement:<category>` 草稿只允许包含一个目标类别数组；补漏不得生成最终 `id`。每条记录沿用对应类别的章节候选或合并字段，并保留 `source_refs`；人物和武功补漏记录仍必须包含合法 `power_rank`。物品类别可以在确定没有合格条目时保留持久化复核：
