...
function pathsFor(novelDir, runId, options = {}) {
  ...
  return {
    ...
    diagnostics: path.join(run, 'diagnostics'),
    workerRootBaseline: path.join(run, 'diagnostics', 'worker-root-baseline.json'),
    workerLeaks: path.join(run, 'diagnostics', 'worker-leaks'),
