...
固定输出沿用合并对象的九类数组和 `candidate_resolutions`，改为 `stage: "cleaned"`，并新增 `quantity_review` 与 `game_material_candidates`。`ambiguities` 必须为空；若仍无法消除歧义，停止提交并转人工，不能猜测。
...
- 清理后重新闭合 `candidate_resolutions`：被删除的候选必须使用有限拒绝原因和非空 detail；保留候选的 `merged_to` 必须仍指向清理数组中存在的实体，不能指向已删除对象。
