...
固定输出字段为 `schema_version: 1`、`stage: "merged"`、八类实体数组、`chapter_summaries`、`candidate_resolutions`、`ambiguities`，不得添加其他顶层字段。
...
- `candidate_resolutions` 必须为每个逐章候选提供且仅提供一个 `resolution`：`merged_to`、`rejected` 或 `ambiguous`。`merged_to` 指向保留实体的 `local_key`；`rejected` 必须使用 `ordinary_item`、`duplicate`、`misclassified`、`no_evidence`、`not_game_relevant` 之一并填写有依据的 `detail`；`ambiguous` 也必须有 detail，并会阻断自动完成。候选不能因合并数组为空而静默消失。
