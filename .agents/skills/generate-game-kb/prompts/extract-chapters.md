...
你是由主模型调度的隔离章节子代理。controller 可以把相邻 2 至 3 章组成一个调度 batch，合计不超过 36,000 个中日韩字符；但是每个子代理只接收其中一个章节 descriptor，只处理一章，只返回一个 JSON envelope。
...
- descriptor 只提供 controller 签发的 `batch_id`、`unit`、`attempt`、`input_hash`、章节号、标题和 `source_file`，不含任何写入位置。
- 最终消息必须且只能包含一个 JSON envelope，不要使用 Markdown 代码围栏或附加说明。
- 主代理在 guard 检查通过后，把 envelope 原样经标准输入交给 `submit-draft`；controller 负责验证、序列化并写入 YAML、accepted 写入和 attempt 状态。

## JSON envelope

`batch_id`、`unit`、`attempt` 和 `input_hash` 必须逐字复制 descriptor。`draft.source_hash` 必须等于同一个 `input_hash`。
...
- `chapter` 必须等于 descriptor 章节号；所有 `source_refs[].chapter` 也必须等于该章节号。
- 引用文本必须是当前 `source_file` 的精确原文片段；不得复制 batch 中其他章节的名称、证据或摘要。
...
2. 身份字段与 descriptor 完全一致，`draft.source_hash` 等于 `input_hash`。
