...
新 V4 run 使用 `semantic_contract_version: 5`、`semantic_profile: domain-distill-v1` 和 `profile: v4`。版本 4 的旧 run 只能查询或显式归档，不能继续写入。
...
Dashboard 直接读取这五个 YAML。完成前还必须看到 `assembly-report.json`、`verification-report.json`、`generate_game_kb_install.json`、`artifact-manifest.json` 和 `archive-receipt.json`，并确认 source hash、final data hash、引用闭包和安装验证一致。
