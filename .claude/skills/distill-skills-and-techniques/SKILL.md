...
```bash
node scripts/preclean/sanitizer.js <小说目录> skills.json skills
node scripts/preclean/sanitizer.js <小说目录> techniques.json techniques --companions '{"skills":"<小说目录>/skills.json"}'
```

检查 `archive/reports/skills.sanitize-report.md` 和 `archive/reports/techniques.sanitize-report.md`，确认自动修复合理。检查 `archive/reports/*.pending.json`，记录待复核项。

预清洗自动完成：
- **skills**：字符串清理、同 id 精确去重、空值规整
- **techniques**：字符串清理、type 枚举归一（13 种标准类型）、source_skill 引用修复、同 name+同 source_skill 去重、orphan technique 补 source_skill（name 完全匹配唯一 skill 时）、空值规整

`archive/raw/` 保留两个原始快照。

---

## Phase 1：清理 techniques

1. 读取已被 Phase 0 预清洗的 `<小说目录>/skills.json` 和 `<小说目录>/techniques.json`
