...
```bash
node scripts/preclean/sanitizer.js <小说目录> characters.json characters
node scripts/preclean/sanitizer.js <小说目录> dialogues.json dialogues --companions '{"characters":"<小说目录>/characters.json"}'
node scripts/preclean/sanitizer.js <小说目录> locations.json locations
node scripts/preclean/sanitizer.js <小说目录> factions.json factions --companions '{"characters":"<小说目录>/characters.json","locations":"<小说目录>/locations.json"}'
```

检查 `archive/reports/*.sanitize-report.md`，确认自动修复合理。检查 `archive/reports/*.pending.json`，记录待复核项。

预清洗自动完成：
- **dialogues**：字符串清理、空 text 删除、感叹词删除、tone 枚举归一、speaker_name 唯一匹配修复
- **locations**：字符串清理、region 关键词映射、one_line 补全、空值规整
- **factions**：字符串清理、type 枚举归一、name+location 精确去重、characters.faction 引用同步、location ID 残留修复、空值规整
- **characters**：（引用修复用，同 Phase 0 characters）

`archive/raw/` 保留四个原始快照。

---

## Phase 1：清理 dialogues.json

1. 读取已被 Phase 0 预清洗的 `<小说目录>/dialogues.json`、`locations.json`、`factions.json`
2. 读取 `characters.json`（用于 speaker 匹配和 faction 归一化）
