...
7. **Sub Agent 必须独立完成以下任务：**
   ...
   **第二步：AI 匹配 speaker + 输出原始数据（ctx_execute 写入临时文件）**
   ```
   ...
   d. 撰写 chapter_summary（约200字）
   e. 识别本章实体（包括未命名角色）
   f. 用 ctx_execute 将所有提取结果写入 batch_json/ch_N_raw.json（临时文件）
   ```

   **⚠️ 未命名角色必须创建 ID。** 即使角色没有真名（如"老丐"、"蒙面人"、"白衣女子"），也要基于当前称呼创建 ID：

   | 当前称呼       | ID 示例                  | 说明                   |
   | -------------- | ------------------------ | ---------------------- |
   | 老丐          | `char_lao_gai`             | 用拼音直译             |
   | 蒙面人         | `char_mian_mian_ren`       | 用拼音直译             |
   | 白衣女子       | `char_bai_yi_nv_zi`        | 用拼音直译             |
   | 黑衣老者       | `char_hei_yi_lao_zhe`      | 用拼音直译             |
   | 无名剑客       | `char_wu_ming_jian_ke`     | 用拼音直译             |

   **后续章节揭晓真名时：**
   - 更新 registry：`name` 改为真名，`alias` 加入旧称呼
   - 后续所有对话的 speaker 使用新 ID

   **⚠️ 关键：必须用 ctx_execute 写入 ch_N_raw.json，不要试图把数据嵌入代码字符串。**

   **第三步：组装并写入最终文件（ctx_execute）**
   ```
   用 ctx_execute 运行 JavaScript：
   - 读取 ch_N_raw.json（已提取的数据）
   - 读取 entity_registry.json
   - 对比识别 new_entities 和 entity_updates
   - 组装 ch_N.json（delta 文件）
   - 更新 entity_registry.json
   - 删除 ch_N_raw.json（临时文件）
   - 验证 JSON 格式正确
   ```
   ...
   **ctx_execute 组装模板：**
   ...
   // 1. 读取已提取的原始数据
   const rawData = JSON.parse(fs.readFileSync('batch_json/ch_N_raw.json', 'utf8'));
   ...
   const delta = {
     ...
     chapter_summary: rawData.chapter_summary,
   ...
   // 4. 识别 new_entities（registry 中不存在的实体）
   for (const type of ['characters', 'skills', 'techniques', 'factions', 'locations', 'items']) {
     for (const entity of (rawData[type] || [])) {
       if (!registry[type].find(e => e.id === entity.id)) {
         delta.new_entities[type].push(entity);
       }
     }
   }
   
    // 5. 识别 entity_updates（已有实体的变化）
    for (const entity of (rawData.characters || [])) {
      const existing = registry.characters.find(c => c.id === entity.id);
      if (existing) {
        const updates = {};
        // 真名揭晓：name 变化时，旧名加入 alias
        if (entity.name && entity.name !== existing.name && existing.name !== '未知') {
          updates.name = entity.name;
          updates.alias = [...new Set([...(existing.alias || []), existing.name])];
        }
        // rank 取 max
        if (entity.rank && entity.rank !== existing.rank) {
          const rankOrder = ['平平无奇','初窥门径','略有小成','登堂入室','炉火纯青','出神入化','登峰造极','返璞归真'];
          const currentIdx = rankOrder.indexOf(existing.rank);
          const newIdx = rankOrder.indexOf(entity.rank);
          if (newIdx >= 0 && (currentIdx < 0 || newIdx < currentIdx)) {
            updates.rank = entity.rank;
          }
        }
        // source_refs 追加
        if (entity.source_refs) {
          updates.source_refs = [...(existing.source_refs || []), ...entity.source_refs];
        }
        if (Object.keys(updates).length > 0) {
          delta.entity_updates.push({ id: entity.id, updates });
        }
     }
   }
   ...
   // 9. 删除临时文件
   fs.unlinkSync('batch_json/ch_N_raw.json');
...
**⚠️ 避免字符串转义问题。** 提取的数据必须通过 ctx_execute 写入临时 JSON 文件（ch_N_raw.json），不要试图把含引号的文本嵌入 JavaScript 字符串字面量。流程：
1. AI 提取数据 → 用 ctx_execute 写入 ch_N_raw.json
2. 另一个 ctx_execute 读取 ch_N_raw.json → 组装最终文件
