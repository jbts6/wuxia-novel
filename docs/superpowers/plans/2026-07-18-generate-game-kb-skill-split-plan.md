...
**Goal:** Ship a repaired v4 audit skill, an isolated Lite fast base skill, and four independent deep-enrichment skills that operate only through hash-bound overlays.

**Architecture:** Keep the existing v4 controller and semantic contract as a read/write audit path, but make worker staging paths controller-owned and rejected drafts reviewable. Add an explicit Lite profile and skill that publishes grounded candidates without domain distillation. Deep skills submit constrained deferred tasks and materialize immutable revisions through the overlay registry.
...
- Lite uses semantic contract version 6 and never requires four domain decision artifacts.
...
- Basic Lite deduplication/action filtering remains deterministic; generic action records stay in quarantine.
...
### Task 2: Add the isolated Lite skill and explicit profile routes

**Files:**
- Create: `.agents/skills/generate-game-kb-lite/SKILL.md`
- Create: `.agents/skills/generate-game-kb-lite/prompts/extract-chapters.md`
...
- Add: `.agents/skills/generate-game-kb/tests/lite-skill-contract.test.js`
- Add: `.agents/skills/generate-game-kb/tests/lite-cli-contract.test.js`

**Interfaces:**
- `createOrResumeRun(novel, { profile: 'lite' })` creates a Lite run namespace and records contract/profile metadata.
- `flow.js` exposes `lite-prepare`, `lite-accept`, `lite-basic-curate`, `lite-publish`, and `lite-status`; v4 commands remain available for v4 runs.
- `generate-game-kb-lite/SKILL.md` documents only the fast base flow and links to deep skills for enrichment.

- [ ] **Step 1: Write RED contract tests** proving the Lite skill exists, Lite commands select profile `lite`, Lite status does not plan domains, and v4 commands reject a Lite run instead of mutating it.
- [ ] **Step 2: Run the tests** and confirm the old shared-command behavior fails these isolation checks.
- [ ] **Step 3: Implement the profile dispatcher and Lite skill wrapper**. Keep the existing Lite normalization, grounding, quarantine, and basic-curate interfaces; do not duplicate controller logic in the skill document.
...
- [ ] **Step 5: Commit** `feat: add isolated generate-game-kb-lite skill`.

### Task 3: Complete grounded Lite assembly and publish
...
- Modify: `.agents/skills/generate-game-kb/scripts/flow.js`
- Add: `.agents/skills/generate-game-kb/tests/publish-lite.test.js`
...
**Interfaces:**
- `assembleRun({ paths, profile: 'lite' })` consumes accepted grounded chapters, the deterministic candidate registry, and optional curate output; it does not consume v4 domain decisions.
- `verifyFinal(paths, { profile })` dispatches to v4 or Lite validation and recomputes accepted, registry, resolution, final, and receipt hashes.
- `publishRun({ paths, profile: 'lite' })` performs assemble, workspace verify, atomic install with installed verification, and archive; each boundary is resumable.
...
- [ ] **Step 3: Implement Lite grounded projection** with nullable enrichment, stable IDs, deterministic source-reference union, and a hard missing-summary error.
...
- [ ] **Step 5: Run Lite and v4 focused suites** and confirm installed verification never falls back to worktree files.
- [ ] **Step 6: Commit** `refactor: publish grounded Lite candidates without domain distill`.
...
**Interfaces:**
- `addDeferredTask({ paths, type, scope, requestedBy }) -> task` binds a pending task to the current Lite base manifest hash.
...
- [ ] **Step 4: Add CLI routes** `task-add`, `task-run`, and `task-apply`; pending tasks must not alter Lite `next_action`.
...
**Interfaces:**
- The skill accepts a published Lite run and character scope, then invokes the `characters-deep` deferred task type.
...
- [ ] **Step 3: Create the skill** with its character prompt, shared overlay constraints, and direct link to the Lite base contract.
...
**Interfaces:**
- The skill accepts a published Lite run and skills scope, then invokes the `skills-deep` deferred task type.
- It produces overlay drafts only and never writes final YAML, accepted evidence, or the base registry.
...
- [ ] **Step 2: Run the tests** and confirm the skill is absent.
- [ ] **Step 3: Create the skill** with its skills/techniques prompt, shared overlay constraints, and direct link to the Lite base contract.
- [ ] **Step 4: Run `quick_validate.py` and the focused contract test**; record the result before moving to the next skill.
...
**Interfaces:**
- The skill accepts a published Lite run and items scope, then invokes the `items-deep` deferred task type.
- It produces overlay drafts only and never writes final YAML, accepted evidence, or the base registry.
...
- [ ] **Step 2: Run the tests** and confirm the skill is absent.
- [ ] **Step 3: Create the skill** with its item-enrichment prompt, shared overlay constraints, and direct link to the Lite base contract.
- [ ] **Step 4: Run `quick_validate.py` and the focused contract test**; record the result before moving to the next skill.
...
**Interfaces:**
- The skill accepts a published Lite run and factions scope, then invokes the `factions-deep` deferred task type.
- It produces overlay drafts only and never writes final YAML, accepted evidence, or the base registry.
...
- [ ] **Step 2: Run the tests** and confirm the skill is absent.
- [ ] **Step 3: Create the skill** with its faction-enrichment prompt, shared overlay constraints, and direct link to the Lite base contract.
...
- Modify: `.agents/skills/generate-game-kb/scripts/flow.js`
- Add: `.agents/skills/generate-game-kb-lite/prompts/publish.md`
...
**Interfaces:**
- v4 documentation describes strict domain distill and bounded retries; Lite documentation describes the fast base and explicitly excludes domain distill.
...
- [ ] **Step 4: Run skill-contract, status, timing, and prompt tests; commit** `docs: separate v4 Lite and deep skill contracts`.

### Task 10: Full regression, E2E, and performance gate

**Files:**
- Modify: `.superpowers/sdd/progress.md`
- Add: `.agents/skills/generate-game-kb/tests/e2e-lite-skill-split.test.js`
...
- [ ] **Step 2: Run focused v4 retry, Lite publish, overlay, deep-skill, status, and timing suites.**
- [ ] **Step 3: Run the complete Node test suite** and record unrelated pre-existing fixture failures separately from regressions.
- [ ] **Step 4: Run the three-chapter Lite E2E flow** and assert exactly five YAML files, quarantine visibility, stable hashes, and no domain artifacts.
...
- [ ] **Step 6: Run the representative 21/50-chapter packing and timing fixtures** and assert the Lite base remains within the 45-minute budget model.
- [ ] **Step 7: Update progress and commit** `test: verify v4 Lite and deep skill split`.
