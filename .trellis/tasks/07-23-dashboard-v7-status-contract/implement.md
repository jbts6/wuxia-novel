...
- 任何 v7 标记存在时不得回退旧 G1-G5，以免损坏的 v7 产物被旧报告误判为通过。
...
- Produce `ValidationContract = 'none' | 'generate-kb-gates' | 'generate-game-kb-v7'`。
...
- [ ] **Step 3: 实现最小适配器**
  ...
  - v7 标记固定为三份报告路径；检测任一存在即进入 v7 分支。
...
- Produce v7 command: `node '.agents/skills/generate-game-kb/scripts/flow.js' status '<book>'`。
...
- [ ] **Step 1: 写动作失败用例**
  ...
  - API 执行 v7 action 时脚本根目录和 `status` 参数顺序正确；
...
- [ ] **Step 3: 集中 presentation 逻辑并更新 UI**
