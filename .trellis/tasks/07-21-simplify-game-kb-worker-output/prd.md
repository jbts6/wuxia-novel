...
- Claude Code 已关闭；《萧十一郎》现有 v6 run 完成约一半。该现场仅用于观察，不属于本任务迁移、续跑或修补范围。
- WorkBuddy 已关闭；《陆小凤传奇》现有 v6 run 已完成，但最终实体未执行有效跨章去重，并曾产生 `.kb-scratch` 下的 raw JSON、清洗脚本和批量提交脚本。该现场仅用于根因证据，不作为新合同迁移输入。
...
- 新合同只用于新 run；现有《萧十一郎》和《陆小凤传奇》v6 run 不迁移、不续跑、不修补、不删除。
- 用户已于 2026-07-21 确认 WorkBuddy 与 Claude Code 均已关闭，不再调用共享的 `flow.js`；实现前现场会话门禁已满足，但仍须完成规划文档评审并显式启动 Trellis 任务。
- 必要的现场清理由用户决定并执行；本任务不得自行删除、移动或修改两个现有 run、`.kb-scratch` 或其他现场文件。
...
- [ ] 新旧合同严格隔离；两个现有 v6 run 的文件与状态保持不变。
- [ ] 实现与测试不得迁移、续跑、修补或改写现有《萧十一郎》《陆小凤传奇》v6 run；用户已确认两个宿主会话均已关闭。
