...
`libraryScanner` 先检查 `generate-game-kb` 管线标记：

1. 存在 `generate_game_kb_install.json`、`verification-report.json` 或 `game-kb-review.json` 中任一项时，进入 `generate-game-kb` 分支。
2. 可读回执的 `semantic_contract_version` 决定 v7 或 legacy 投影；文件名本身不能证明版本。非 v7 回执标记为待迁移。
3. `generate-game-kb` 分支不回退 G1-G5；即使目录残留旧 `quality_report.json` 也不得兜底通过。
4. 没有 `generate-game-kb` 标记时，维持现有 `parseQualityReport` 行为。

扫描结果新增合同来源和验证 warning，保留现有 `validationStatus` 以减少 API 破坏：

- `validationContract`: `none | generate-kb-gates | generate-game-kb-legacy | generate-game-kb-v7`
...
v7 失败且安装回执提供 run id 时，建议动作调用 `generate-game-kb/scripts/flow.js status <novel> --run <run-id>`；缺少 run id 时不猜测当前 run。展示命令与 API executor 共用一个结构化调用构造。旧流程继续使用 `assess-quality.js`。v7 已通过但含 warning 时不产生补救动作，只展示提示。
...
- `generate-game-kb` 标记存在但回执缺失、JSON 损坏、哈希不一致或验证器异常：明确判为 failed，不能回退旧合同。
