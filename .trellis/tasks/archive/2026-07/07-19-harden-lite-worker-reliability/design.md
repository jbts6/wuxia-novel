...
```json
{
  ...
  "attempt": 1,
  "input_hash": "sha256:<controller value>",
  "draft": { "schema_version": 1, "chapter": 1 }
}
```

`draft` 是 V6 run 中待验证的数据对象，不是已经接受的 artifact；它自己的 chapter schema 版本仍为 `schema_version: 1`。实际 `.yaml` 字节只能由 controller canonical writer 生成。
