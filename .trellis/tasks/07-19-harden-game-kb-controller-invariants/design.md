...
broker 必须支持 fault injection。相同 unit/attempt/raw hash 在 `receipt-created`、`staging-written`、`submission-recorded` 或 `accepted-written` 后重放时恢复或返回同一结果；不同 raw hash 的重放返回 `SUBMISSION_REPLAY_CONFLICT`，不得覆盖任何字节或再消费一次 attempt。stdin 有明确字节上限，拒绝空输入、NUL 和额外 envelope。
