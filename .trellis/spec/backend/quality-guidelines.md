...
- The controller groups adjacent chapters into dynamic worker jobs of two or three chapters with at most 36,000 CJK characters. Oversized chapters and an unpairable tail may be single-chapter jobs. Every chapter remains an independent `chapter:NNN` unit. Controller-internal descriptors retain one absolute, run-scoped, attempt-specific `staging_path`, but worker projection must expose no staging path, output directory, output filename, or other writable target; `worker_write_paths` is always empty. Workers return an envelope to the submission broker instead of writing YAML. `古龙/剑神一笑` is the real-corpus fixture: 20 chapters and job sizes `[3, 3, 3, 3, 3, 3, 2]`.
...
- Bad: deriving a batch ID from `chapter:NNN`, reusing a guard whose immutable check recorded a violation, or dispatching from a legacy run because accepted files happen to parse.
...
- Lite controller/worker safety: assert Controller-issued multi-chapter batch identity survives guard and broker submission, immutable violation receipts remain unchanged while status filters restored paths, violated guards cannot submit, a new clean guard can continue, and legacy accepted serialization exposes no jobs or Lite write route.
