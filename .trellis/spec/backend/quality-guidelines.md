...
Normal V4 stage order is `archive-existing -> prepare -> chapter:NNN accept -> plan-domains -> four distill:* accept -> assemble -> verify -> install -> verify --installed -> archive-run`. Lite uses `lite-prepare -> chapter:NNN accept -> lite-plan-domains -> lite-publish`; the current main model owns routing and serial acceptance, and the user supplies only the novel directory.
...
- Guard receipts are immutable audit evidence, while unresolved status and broker eligibility are the same read-only projection over current repository state. Removing an added rogue path or restoring a changed path clears that violation without rewriting the original check receipt. Existing directories retain identity by type; child writes may change directory size/mtime as derived metadata, which is neither a new violation nor an unresolved historical violation. Added, deleted, or type-changed directories and every added, deleted, modified, or type-changed file remain blocking.
...
- Lite must not bypass candidate planning. After all Lite chapters are accepted and before a current candidate registry/domain plan exists, `lite-status` returns the executable `lite-plan-domains` alias. That controller-owned command creates or validates the immutable registry and plan. Once they are current, Lite has zero required full-book domain units, exposes no `domain_jobs`, and `lite-status` returns `lite-publish`. V4 retains all four required domain units and its unprefixed `plan-domains` action.
...
- A guard check with any currently unresolved write violation -> `GUARD_VIOLATIONS_UNRESOLVED` for status and broker submission. Removing/restoring the exact path clears the dynamic block without changing receipt bytes; directory size/mtime-only drift is ignored.
...
- Good: a two-chapter Lite guard keeps the Controller-issued batch ID for both per-chapter envelopes, survives repeated status reads without identity drift, and submits only when its immutable check receipt has no currently unresolved violation.
...
- Bad: assigning multiple chapters or multiple domains to one Worker, exposing a writable path, asking a Worker to write YAML, deriving a batch ID from `chapter:NNN`, submitting while a guard violation is still unresolved, or dispatching from a legacy run because accepted files happen to parse.
...
- Lite controller/worker safety: assert Controller-issued multi-chapter batch identity survives expansion into single-chapter assignments and broker submission, every chapter/domain Worker has `worker_write_paths: []`, immutable violation receipts remain unchanged while status and broker share dynamic unresolved-path semantics, directory metadata-only drift is ignored, unresolved guards cannot submit, and legacy accepted serialization exposes no jobs or Lite write route.
- Lite lifecycle routing: assert `lite-status` returns `lite-plan-domains` before registry creation, executing the alias creates an immutable controller-owned candidate registry, the next status returns `lite-publish` with no domain workers, and V4 still requires all four domain units.
