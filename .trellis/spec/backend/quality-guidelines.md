...
AI units are `chapter:NNN`, `merge:<category>:<shard>`, optional `merge:<category>:consolidate`, `clean:<category>:<shard>`, `clean:materials:001`, `quality:sample`, `recall:<category>`, and `supplement:<category>`. `merge:book` and `clean:book` are deterministic aggregate units with `attempts: 0`, never AI drafts. `status` is observational and never returns an executable next action.

### 3. Contracts

- Request: one source novel under `<novel-dir>`; `archive-existing` first leaves only that source, `ch_split/`, and `_archive/`. Each run lives under `.game-kb-work/runs/<run-id>/`; `prepare` derives ordered chapter files and a hash-bound manifest, creates or restores `<run-dir>/staging/`, and resuming skips unchanged completed chapters. Every new run persists `run.json.semantic_contract_version: 2`.
...
- Chapter drafts use provisional local keys and name-based references. Category merge/clean/material drafts see only bounded short references and semantic facts. Controller-only `candidate_key`, `local_key`, private bindings, full candidate ledgers, unchanged whole-book arrays, and final IDs are forbidden in those AI-visible inputs and outputs.
...
- Merge work is deterministically grouped by category and stable name order, then sharded at both 120 candidates and 96 KiB serialized AI input. Short references, shard boundaries, files, private bindings, and input hashes must be byte-stable for identical inputs. More than one shard, or a same-name group split by a limit, creates a summary-only consolidation unit.
- Every merge member ref is decided exactly once. The controller expands private bindings, creates stable whole-book local keys, unions source refs, projects chapter summaries in manifest order, and closes every candidate as exactly one `merged_to`, finite-reason `rejected`, or blocking `ambiguous` decision before `assemble-merge` writes the compatible merged book.
- `prepare-clean` derives stable obligations for ambiguity, detailed-character requirements, over-expanded minor characters, item inclusion, dialogue event/chapter/uniqueness, ledger closure, and book-contract errors. Every entity ref receives exactly one `keep`, `edit`, `merge_into`, or finite-reason `drop` decision. Named skills/techniques and core/important characters cannot be dropped.
- Clean ledger migration is controller-owned: keep/edit preserves candidate destinations, merge_into redirects them, and one entity-level drop reason mechanically rejects every mapped candidate. Keep-all is valid only when obligations are empty, every entity is decided, and the resulting book contract passes. Materials are selected afterward from a compact surviving-entity catalog; `assemble-clean` writes the compatible cleaned book with `attempts: 0`.
- Deterministic coverage or resolution gaps may open only one bounded `recall:<category>` or `supplement:<category>` semantic generation plus one format-only correction; they never create a full-book retry. Quantity is advisory and cannot become a completion gate. Deterministic planning or assembly errors consume no AI attempt and never trigger reset.
...
- Missing or different `semantic_contract_version` on a writable path -> `LEGACY_SEMANTIC_CONTRACT`; preserve status and evidence, never upgrade or reset the run in place. Abandoned archival without confirmation -> `ABANDON_CONFIRM_REQUIRED`.
- Category draft includes `candidate_key`, `local_key`, a final ID, or any private binding -> `MECHANICAL_KEY_FORBIDDEN`. A short ref is missing, duplicated, outside its work item, or crosses categories -> `WORK_REF_INVALID`.
...
- Named skill/technique or core/important character is dropped -> `PROTECTED_ENTITY_REMOVAL`. A claimed clean obligation remains present -> `CLEAN_OBLIGATION_UNRESOLVED`.
- Merge ambiguity remains, a category is incomplete, or the full candidate ledger does not close -> `MERGE_AMBIGUITY_UNRESOLVED` or `BOOK_ASSEMBLY_INCOMPLETE`; no aggregate book is written.
...
- Good: every chapter is accepted, candidate destinations and accepted hashes verify, any deterministic category gap is resolved once, bounded category merge/clean decisions assemble attempts-0 books, named martial material is retained, ordinary actions and trivial items are absent, fixed non-reallocating quotas pass, clean installation succeeds, installed data re-verifies, and the complete run archives.
...
- Bad: silently adding version 2 to an unversioned run, using its successful-looking output as positive evidence, installing it, or discarding its failed drafts without explicit abandoned-archive confirmation.
...
- Category contract: AI-visible short refs only; mechanical-key exclusion; exact member/entity coverage; legal action/reason/patch fields; stable 120-candidate/96-KiB shards; summary-only consolidation; and byte-identical replanning.
- Deterministic assembly: 1,089 short-ref candidates expand to one resolution each; one entity-level drop mechanically migrates 420 candidates; clean obligations reject keep-all bypasses; named techniques and detailed characters reject direct drop; category failure leaves accepted siblings unchanged; and both aggregate books keep attempts zero.
