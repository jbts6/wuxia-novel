# Pass 1 — 《侠客行》实体骨架生成 prompt

## 角色

你是一位资深的**武侠小说**研究者兼数据库工程师，尤其精通金庸作品。你已经熟读《侠客行》，现在需要基于原文写一份完整、准确、无冲突的设定库。
...
《侠客行》采用金庸典型的典雅白话风格，叙事节奏张弛有度。语言特色：
- 对话文雅而不失生动，角色性格通过对话鲜明体现
- 武功描写注重意境与哲理，如太玄经的领悟过程
- 情节推进以石破天的身份错位为主线，穿插多条副线

### 关键事件时间线

基于 mention_summary 的高频事件词推断的全书重大事件节点：

1. **第1-3章**：玄铁令出现，石破天初入江湖（first_mention 关键期）
2. **第4-6章**：长乐帮事件，石破天被误认为帮主（climax 之一）
3. **第7-10章**：雪山派冲突，凌霄城事件（climax 之二）
4. **第11-14章**：赏善罚恶令出现，各派反应（背景铺垫）
5. **第15-18章**：前往侠客岛，腊八粥事件（resolution 前奏）
6. **第19-21章**：侠客行石壁，太玄经领悟（resolution）

### source_ref 锚定策略

- **first_mention**：石破天首次出场（第1章）、玄铁令首次出现（第1章）、雪山派首次提及（第1章）
- **climax**：长乐帮帮主身份确认（第4-5章）、凌霄城大战（第7章）、侠客岛石壁领悟（第20章）
- **resolution**：石破天身世揭晓（第18章）、太玄经练成（第21章）
- **background**：各门派背景介绍、武功渊源说明
...
3. **关系图常见错误**：
   - 石破天与石中玉的关系容易混淆（孪生兄弟但品行迥异）
   - 丁珰与石破天的关系有多个阶段（从利用到真心）
   - 白自在与史婆婆是夫妻关系，但分居多年
...
- 常见错误：`"长乐帮"`、`"雪山派"` 等是 faction，应使用 ID。

**characters.known_skills / related_skills**：
- 必须使用 `skills.json` 中的 `id`（如 `skill_tai_xuan_jing`），**不能**使用名称（如"太玄经"）。
...
7. **最后检查**：
   ...
   - **禁止使用名称代替 ID**：如"雪山派"应为 `faction_xue_shan_pai`，"太玄经"应为 `skill_tai_xuan_jing`
