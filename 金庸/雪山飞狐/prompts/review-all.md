# Knowledge Base Review Prompt（雪山飞狐专属）
...
- **武器混入**：是否将武器（胡家刀、苗家剑等）误列为功法？
  - **区分标准**：
    - skills.json 应包含：胡家刀法、苗家剑法、八卦刀、天龙剑法等**武学体系**
    - items.json 应包含：胡家刀、苗家剑等**武器/道具**
  - **常见错误**：将"胡家刀"（武器）列为 skill，应为 item
...
- **武器错放**：是否有功法被误列为物品？
  - **区分标准**：
    - items.json 应包含：胡家刀、苗家剑、凤头珠钗等**武器/道具**
    - skills.json 应包含：胡家刀法、苗家剑法等**武学体系**
- **地点错放**：是否有地点被误列为物品？
  - **区分标准**：
...
2. **skills.json 混入武器**：胡家刀、苗家剑等应为 item 而非 skill（severity: high）
3. **items.json 混入地点**：玉笔峰、长白山等地点应为 location 而非 item（severity: high）
4. **items.json 混入功法名称**：胡家刀法、苗家剑法等功法名称（无实体秘籍）应为 skill 而非 item（severity: high）
